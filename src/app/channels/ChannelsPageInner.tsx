...
export default function ChannelsPageInner() {
  const [channelsByGroup, setChannelsByGroup] = useState<Record<string, Channel[]>>({})
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch('/channels/playlist.m3u').then(r => r.text()),
      fetch('https://iptv-org.github.io/iptv/index.category.m3u').then(r => r.text()),
    ]).then(([localText, remoteText]) => {
      const allText = localText + '\n' + remoteText
  ...
  const setGrouped = (channels: Channel[]) => {
    ...
    setSelectedGroup(Object.keys(grouped)[0] || null)
  }
  ...
  const addChannel = () => {
    ...
    const updated = [...(channelsByGroup['未分类'] || []), newChannel]
        ...
        <div className="flex-1 flex flex-col sm:flex-row gap-4 overflow-hidden">
          ...
          <div className="w-full sm:w-[220px] flex flex-col border rounded-lg bg-gray-50 dark:bg-gray-900">
            <div className="flex justify-between items-center px-2 py-2 border-b">
              ...
              >
                {Object.keys(channelsByGroup).map(group => (
            ...
            <ul className="flex-1 overflow-y-auto px-2 py-2 space-y-2">
              {channelsByGroup[selectedGroup || '']?.map((ch, idx) => (
                <li key={idx}
                  ...
                  onClick={() => play(ch)}
                >
                  {ch.logo && <img src={ch.logo} alt="logo" className="w-5 h-5 object-contain rounded" />}
          ...
          <div className="flex-1 flex flex-col space-y-4 overflow-hidden h-full">
            <div className="flex-1 border rounded-lg overflow-hidden h-full min-h-[300px]">
              {currentChannel ? (
                <ArtPlayer url={currentChannel.url} title={currentChannel.name} isLive={true} blockAd={true} />
              ) : (
